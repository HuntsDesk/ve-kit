...
ve-kit is distributed from a private upstream repository via `git subtree`. The canonical source is a `docs/ve-kit/` directory inside that repo; `HuntsDesk/ve-kit` is a force-pushed squashed mirror of it. This means:

- PRs against `HuntsDesk/ve-kit` can't be merged directly (force-push clobbers them). Instead, the maintainer pulls the changes upstream and republishes.
