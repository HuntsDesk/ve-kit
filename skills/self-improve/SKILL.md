...
1. **Review findings** — query the board activity log for review output over the window (last ~5 batches or ~14 days):
   - `board_get_activity` filtered to `action: "commented"` by `code-reviewer` / `test-runner` / `processor` / review-mode agents. Since 2026-09-04 medium/low findings live ONLY as processor-authored ledger comments on the severity parent (never as tasks), so omitting `processor` here silently drops the whole medium/low pattern stream.
...
4. **Gate telemetry** — first `board_query_gate_events(since=<window start>)` (the shared Firestore `gate_events` collection, ALL developers, fed automatically by the SessionStart/Stop ingest hook since 2026-09-16), then `.claude/telemetry/gate-events.jsonl` on this machine as the fallback and as a check that this machine's lines were ingested (`scripts/ingest-gate-events.sh --count`):
   - Report per-dev counts. A dev with zero events either had `VE_TELEMETRY_INGEST=off`, opened no new windows since the hooks landed, or has an ingest failure in `/tmp/ve-telemetry-ingest.*.log` — say which, do not read zero as clean.
   - **Read `.claude/telemetry/.ingest-state.json` before trusting any count.** It carries `last_success_at`, `last_status`, `last_error`, `written_total` and `quarantined_total`, and exists precisely so a dead ingest cannot go on looking like an idle one — a stale `last_success_at`, a `last_status` of `partial`/`error`, or a non-zero `quarantined_total` means this machine's numbers are incomplete, and you say so instead of reporting the total. Quarantined rows sit whole (never truncated) in `.claude/telemetry/gate-events.deadletter.jsonl` with a reason — read them, they are events the shared store will never receive.
   - `bash-edit` / `edit` events are the tool-independent edit record (files changed through Bash heredocs, `sed -i`, redirects). Compare their volume against `fact-gate` denies: the gap is the share of edits the demand-first gate never saw.
   - **Count bash-edit rows with `part` absent or `part == 1` (or dedupe on `session_id` + `ts`) — never raw row count.** Since 2026-09-19 a single Bash call whose changed-path list exceeds the per-row budget is emitted as SEVERAL rows carrying `part`/`parts`, so `by_hook_event["bash-edit/edit"]` counts one call as N edits and silently inflates exactly the numerator you are comparing against fact-gate denies. Read `n` (the total, identical on every part) for how many FILES changed; count deduped rows for how many CALLS.
   - Count denies by `hook` and by `path`. A path that trips `fact-gate` in ≥2 distinct sessions, or a protected file overridden in ≥2 sessions, is a candidate recurring pattern at Step 3's evidence bar.
   - **A deny is not a catch.** Before counting one, check that session's board activity: did the fact-gate retry actually carry an importer list, or did the agent restate the instruction and move on? The second shape is a finding about the GATE — it cost a round-trip and changed nothing — not about the code.
   - `VE_GATE_TELEMETRY=off` means a machine recorded nothing; say so rather than reading it as a clean window.
