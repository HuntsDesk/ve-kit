---
name: review-all
description: Orchestrator that runs the full review-* family (rules, agents, skills, docs, memory, board, security) and aggregates findings under a single board parent task. Produces a prioritized remediation roadmap. Use for full system audits.
...
6. `/review-board` — Vibe Board state (Firestore via `mcp__vibe-board__*`)
...
[review-all] System Audit <date>
  status: in_progress
  priority: high
  description: Orchestrated run of review-rules, review-agents, review-skills, review-docs, review-memory, review-board, review-security.
```
...
- Naming inconsistencies across agents/skills (e.g., `billing-*` vs `bill-*`)
