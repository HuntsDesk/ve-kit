...
The roster, each hook's event and matcher, its escape hatch and its test suite are in [`.claude/hooks/README.md`](../../hooks/README.md) — read that, then `ls .claude/hooks` to confirm it is current. Verify each hook fails CLOSED (deny on unexpected input), not OPEN — **except** the compliance gates, which fail OPEN by design on a missing python interpreter or unparseable hook input, because a gate that hard-denies every edit when its interpreter is absent bricks the session; board `<YOUR_HOOK_DOCTRINE_TASK_ID>` tracks reconciling that doctrine split, so note deviations rather than re-litigating it here. Note too that the edit-time gates only see the `Edit|Write|MultiEdit` tools — a Bash heredoc or `sed -i` rewrite bypasses them by construction, with the commit-time review gate as the backstop — so that bypass is a boundary of the control, not a finding.
