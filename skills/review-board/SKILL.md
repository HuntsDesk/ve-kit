---
name: review-board
description: Review the Vibe Board (Firebase Firestore) for stale tasks, orphans, duplicates, done-but-unmarked work, abandoned projects, session handoff quality, and referential integrity. Files every finding to the board (tasks for high/critical, a ledger comment for medium/low).
...
Audits the Vibe Board state itself — the meta layer where all other work is tracked. Uses `mcp__vibe-board__*` tools to query live Firestore data.
...
Archive via `board_update_project(status: "archived")` — vibe-board 2.0 supports it, so archive rather than leaving a comment.

Two other 2.0 affordances belong in this sweep. `board_bulk_update_tasks` closes a batch of stale tasks in one call instead of N. And the `on_open_children` guard means `board_update_task(status="done")` **rejects** closing a parent that still has open children unless you pass `close_all`, `detach`, or `leave_attached` with a reason — so a run of `leave_attached` reasons that read as bypass excuses ("TODO later", "just close it") is itself a finding.
...
## Step 8a — Parked stashes (local, invisible to the board)

`git stash list | grep -E '^stash@\{[0-9]+\}: On [^:]+: close/'` on this machine (anchored: a branch named `close/*` or a message mentioning a `close/` path is not a /close stash). Every `close/` stash must have an open `[CHORE] Pick up stash close/…` board task naming it; one older than 3 days with no task is a finding (file the task or, if the work is superseded on HEAD, drop the stash and say so). This check only sees the machine it runs on — say which machine in the report.
...
For each **high/critical** finding, create a subtask (medium/low findings go into ONE ledger comment on the severity parent — `.claude/rules/agent-board.md` § "Review findings → board tasks", severity-gated since 2026-09-04) with:
...
- `mcp__vibe-board__board_create_task`, `board_update_task`, `board_log_activity`
- `mcp__vibe-board__board_get_handoff` (for session quality checks)