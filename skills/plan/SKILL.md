...
**If 3-tier is selected**: invoke `project-coordinator` to produce the STORY task + DESIGN task per its 3-Tier Decomposition Protocol, THEN return here for Step 2 to decompose the design into implementation tasks. Set `design_id: <design_task_id>` on each implementation task so reviewers can trace back, and `parent_task_id: <design_task_id>` for the hierarchy link.

**Set the native `task_type`** on every task you create — `story`, `design`, `task`, `bug`, `chore`, `investigation` — plus `story_id` / `design_id` for linkage. Don't put the type in the title; legacy tasks that carry a `[STORY]` / `[DESIGN]` / `[BUG]` / `[CHORE]` / `[INVESTIGATION]` prefix predate the field. See `.claude/rules/agent-board.md` § Task Type.
...
- `title`: Clear, actionable description — no `[TYPE]` prefix, the field carries the type
- `task_type`: `task` for implementation work descending from a design; `bug` / `chore` / `investigation` for flat work
- `design_id` / `story_id`: set when the task descends from a design or story (3-tier flow)
...
| Marketing copy, UTM naming, analytics | `marketing-specialist` |
| Ad tracking, click IDs, conversion uploads | `paid-ads-specialist` |
...
| Unleash feature flags — taxonomy, rollout, retirement, inventory | `flag-manager` |
| Institutional channel — instructor console, institutions, cohorts, accreditation exports | `institutional-specialist` |
| Legacy product line — its surfaces, entitlement, and routes | `legacy-product-specialist` |
| Vertex AI Memory Bank — persona engines, MemoryProvider, output filter | `memory-bank-specialist` |
| Classic assessment content authoring — items, prompts, tasks | `assessment-writer` |
