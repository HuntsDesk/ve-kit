...
- **Commit = publish.** Under the distributed push model, any other window's push sweeps every committed change on shared `dev`, so commit only deploy-safe units: feature-flag the risky surface, apply the migration first, or hold the change back.
