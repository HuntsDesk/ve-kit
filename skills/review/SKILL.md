...
### What counts as EXECUTE output

**Prod-state mutations count even when `git diff` is empty or trivial.** DB DDL / grants / index / matview changes applied via psql, n8n workflow edits via API or UI, live config edits and reloads, LB / url-map imports, Secret Manager writes, and feature-flag toggles are all reviewable output. Give `code-reviewer` the operational context (what was applied, where, evidence) alongside any repo diff. The 2026-07-20 miss: a session mutated prod five ways with ~2 lines of repo code, skipped review because nothing matched the LOC-based triggers, and the retroactive review caught a HIGH-severity password exposure.

### Who you may invoke without asking

`code-reviewer` and `test-runner` **MAY be invoked unprompted** on entering REVIEW — do not ask permission first, including after prod-state mutations with no repo diff. **Every other agent** needs the user to ask, or to accept an explicit offer ("I'd have content-writer look at X — want that?"). This is the 2026-08-04 founder decision (board `<YOUR_DECISION_TASK_ID>`); do not re-derive it or re-flag it as open. When a judgment belongs to a domain owner you cannot delegate to, say so in the deliverable rather than quietly substituting your own call.

Authoritative statement of the review gate: `.claude/rules/riper-cat.md` MODE 5. Read it there rather than relying on a restatement here.
...
- **Ad tracking / conversion upload code changed**: → `paid-ads-specialist`
...
- Processor creates board tasks for **high/critical** findings only (one subtask each under a per-tier parent) and writes **medium/low** findings as one ledger comment on the parent — per `.claude/rules/agent-board.md` § "Review findings → board tasks" (severity-gated since 2026-09-04)
- Processor returns lean summary

### Step 4: Composite Verdict

- **PASS**: All sub-agents report no critical/high issues. Ready for COMMIT. **Create the review gate marker** so the commit hook allows the next `git commit`. The marker is **session-namespaced** — `/tmp/ve-review-complete.<session_id>`, not the bare path. Create it with `touch "/tmp/ve-review-complete.$CLAUDE_CODE_SESSION_ID"` (that env var is exported into every Bash call and equals the hook's session id), which avoids a deny round-trip; if the gate does deny, its message prints the exact resolved path — `touch` that verbatim.
