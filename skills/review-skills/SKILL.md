---
name: review-skills
description: Review all skill definitions in .claude/skills/ for frontmatter correctness, progressive-disclosure hygiene, trigger clarity, merge/split candidates, and gap analysis. Files every finding to the board (tasks for high/critical, a ledger comment for medium/low).
...
- Parse frontmatter. The full supported field set is `name`, `description`, `when_to_use`, `argument-hint`, `disable-model-invocation`, `user-invocable`, `allowed-tools`, `model`, `effort`, `context`, `agent`, `hooks`, `paths`, `shell` — see `../_shared/anthropic-configuration-guide.md` §2 for what each does. Any key outside that set is an invented field and a finding.
- Count body lines
...
Create parent tasks per severity + subtasks for high/critical findings (medium/low go into ONE ledger comment on the severity parent — `.claude/rules/agent-board.md` § "Review findings → board tasks", severity-gated since 2026-09-04) on the current board project (default: `Agent Infrastructure`, `<YOUR_PROJECT_ID>`). Assigned agent: `docs-manager` for most; `code-reviewer` for skills with structural issues.
