...
| Field | Notes |
|-------|-------|
| `model` | `fable`, `opus`, `sonnet`, `haiku`, `best`, `inherit` (default), or a pinned ID (`claude-opus-5`, `claude-fable-5-1`). **Prefer the alias.** `opus` resolves to the current Opus (`claude-opus-5` since 2026-07-27) and `fable` to the current Fable (`claude-fable-5-1` since 2026-09-01), so alias-based agents inherit each new release for free; a version-pinned ID silently strands the agent on an old model. `inherit` takes the session's model; `best` lets Claude Code pick the strongest available. |
...
| `effort` | `low`, `medium`, `high`, `xhigh`, `max`. Overrides session effort when present. **When omitted the agent INHERITS the session's effort — it does not fall back to a fixed `medium`.** Claude Code's own default is `high` on both Fable 5.1 and Opus 5. Set it explicitly on every Fable agent (all but `processor`): **`high` by default**, `xhigh` only where the deliverable is a decision/diagnosis rather than a document, `max` only for the two domain-content authors. |
...
## 3. Model assignment rubric (Opus 5 / Fable 5.1 / Sonnet 5 / Haiku 4.5)
...
> **Claude Opus 5 (2026-07-27):** `model: opus` resolves to **`claude-opus-5`** — same $5/$25 pricing as Opus 4.8, so this was a free capability upgrade. Because agents use the **alias** rather than a version-pinned ID, the whole Opus fleet picked it up with **zero frontmatter edits**.
>
> **Claude Fable 5.1 (2026-09-01):** `model: fable` now resolves to **`claude-fable-5-1`** — same $10/$50, with cache reads at $0.25/MTok. Again zero frontmatter edits. Two of the July rationales for moving agents off Fable are now **retired**: the "Opus 5 within 0.5% of Fable's CursorBench" parity claim (Fable 5.1 scores CursorBench 73.4 vs Fable 5's 70.5, Terminal-Bench 55.8 vs Fable 5's 42.0 — note Opus 5 scores 52.3 on that row, so do not read 42.0 as an Opus figure, and Terminal-Bench-Science 52.6 against Opus 5's 29.0), and the cyber-classifier objection (Fable 5.1 intervenes ~60% less often per session in Claude Code, and vulnerability-finding is now permitted). The third rationale, **capacity**, was retired by founder directive on **2026-09-05** (quality over capacity — paraphrased: "a founder's time is worth more than capacity"). Fable 5.1 is now the **default**, carrying nearly every agent; Opus 5 keeps only agents that transcribe rather than author or decide. (A hard quota wall can still force a temporary fallback; `.claude/agent-model-tiers.json` records a preferred and a fallback pair per agent so the restore is exact — `agent-model-tier.py` and a template manifest ship in the kit.) The rule that derives that roster — and per-agent membership — lives in review-checklist.md.

| Model | Strengths | Cost (input/output per MTok) | Recommended for |
|-------|-----------|------------------------------|-----------------|
| **Opus 5** | State-of-the-art coding + long-horizon agentic work; more than doubles Opus 4.8 on Frontier-Bench v0.1. High precision *and* recall on code review / bug-finding. Most aligned Claude model to date. 1M ctx, full effort ladder. `low`/`medium` unusually strong. Trails Fable 5.1 on every published agentic benchmark. | $5 / $25 | Only agents that transcribe rather than author or decide — today `processor` alone. Separately, an unattended batch runner (ve-worker) may be defaulted to `opus` per project — a fixed default outside the preferred/fallback switch, in `docker/ve-worker/docker-compose.yml`, not a `.claude/agents/*.md` roster seat. |
| **Fable 5.1** | Strongest on general intelligence and demanding open-ended reasoning; named strengths include code review, debugging, and sustained async sub-agent coordination. CursorBench 73.4 (Opus 70.0), Terminal-Bench 4.0 55.8 (52.3), Terminal-Bench-Science 52.6 (29.0). 1M ctx, full effort ladder; lower efforts exceed prior models at `xhigh` (Anthropic effort guidance, not the launch page). Capacity capped at 50% of Max limits, but **capacity is no longer a tiering input** (2026-09-05); requires 30-day data retention (no ZDR). | $10 / $50 | **The default** — nearly every agent, effort-tiered. See review-checklist.md. |
...
| Last-gate code review (code-reviewer) | `fable` + `effort: high` — a review is a long prose deliverable; see the ~2× draft-twice caution |
| **Decision/diagnosis** deliverables where under-thinking costs more than overthinking — money/auth, schema + data integrity, outage-class infra, deploy, cross-system AI, plus prod flag toggles — flag-manager, and ad conversion/PII upload routing — paid-ads-specialist (members: review-checklist.md tier rows) | `fable` + `effort: xhigh` |
| **Long-prose** deliverables — reviews, audits, docs, copy, content, and every product-domain specialist (members: review-checklist.md tier rows) | `fable` + `effort: high` (the default) |
| Transcribes rather than authors or decides (processor only) | `opus` (effort omitted — inherits the session, default `high`) |
| Read-only exploration (future `Explore`-type agents) | `haiku` |

**Rule of thumb (rewritten 2026-09-05)**: **Fable 5.1 is the default, at `effort: high`.** An agent stays on Opus only if it transcribes rather than authors or decides — no product/config writes, no prod mutation, no user-facing claim, no gate verdict. **Cost and capacity are not valid reasons to keep an agent on Opus.** **Every Fable agent states `effort:` explicitly**, because omitting it hands the agent's ceiling to whatever effort the launching session happens to be running at; raise to `xhigh` only for decision/diagnosis work, since at higher effort Fable 5.1 may draft a long deliverable in thinking and write it again (~2× output tokens). **Use the alias, never a version-pinned ID** — aliases are what made both the Opus 5 and Fable 5.1 upgrades zero-edit events.
...
- **`effort: xhigh`** — effort level between `high` and `max`. Anthropic's recommended default for coding/agentic work on Opus 5, but on Fable 5.1 the recommended start is `high`, so the reference fleet reserves `xhigh` for decision/diagnosis deliverables (members in review-checklist.md's tier rows; the only stated count is its **Preferred roster total** line).
- **Claude Opus 5** (adopted here 2026-07-27) — `model: opus` / `claude-opus-5`. State-of-the-art coding + long-horizon agentic work at unchanged Opus pricing ($5/$25); most aligned Claude model to date. Since 2026-09-05 runs only the transcribe-not-author agents (`processor`) by preference. See model rubric §3.
- **Claude Fable 5.1** (5.0 introduced 2026-06-09, made permanent 2026-07-18 at 50% of Max plan limits; 5.1 released 2026-09-01) — `model: fable` / `claude-fable-5-1`. Leads on general intelligence, code review, debugging, and terminal/agentic work. **The default model since 2026-09-05**, preferred for nearly every agent after the founder withdrew capacity as a tiering constraint. Capacity was withdrawn as a *tiering* input, not as a physical limit: a weekly quota wall can still force a temporary fallback to Opus. See model rubric §3.
