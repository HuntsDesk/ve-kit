...
`review-checklist.md` is authoritative — do not restate its tier table here, and do not invent reassignments it does not sanction. **Fable 5.1 is the fleet default since 2026-09-05** (`opus` carries only transcribe-not-author agents such as `processor`), so an agent on `fable` is NOT a finding. **A quota wall can put a fleet in a temporary Opus fallback** — run `agent-model-tier.py status` FIRST, and while `.claude/agent-model-tiers.json` declares `fallback`, an agent on `opus` is **not a finding either**. Compare against the manifest, never against a remembered count. Sonnet is Reserved, so "move this routine specialist down to Sonnet" is **not** a valid finding either.
...
- An agent whose deliverable is a **long prose document** (reviews, audits, docs, copy, content) sitting at `xhigh`/`max` → lower to `high`; at higher effort Fable 5.1 tends to draft in thinking and write again (~2× output tokens)
- A dated model ID pinned in `model:` instead of an alias → always a finding; `fable` and `opus` must resolve to the current release
- `inherit` on any agent other than `processor` → pin explicitly
