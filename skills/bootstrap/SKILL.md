...
| 3 | Write rule files: `riper-cat.md`, `code-quality.md`, `documentation.md`, `git-workflow.md`, `agent-board.md`, `multi-window-coordination.md` (the last one unless the user runs a single window only) |
| 4 | Set up Vibe Board (Firestore project, service account, MCP server, `.mcp.json`) |
| 5 | Install hooks — `01-BOOTSTRAP.md` Phase 5 carries the set the kit ships. For what each hook in THIS repo does, its escape hatch and its test, see [`.claude/hooks/README.md`](../../hooks/README.md); re-list `.claude/hooks/` rather than trusting any written enumeration, kit and repo differ |
...
- MCP board build succeeds (`npm run build` in `ve-vibe-board/` — cloned from `HuntsDesk/ve-vibe-board` during Phase 4)
