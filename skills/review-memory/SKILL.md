---
name: review-memory
description: Review the per-project auto-memory directory at ~/.claude/projects/<slug>/memory/ for index currency, stale entries, duplicates, type correctness, and uncaptured patterns. Files every finding to the board (tasks for high/critical, a ledger comment for medium/low).
...
- Memory should reference current terminology (Opus 5, not 4.8/4.7)
