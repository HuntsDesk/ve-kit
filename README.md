...
| Nothing enforces the process gate between "wrote code" and "committed code." Reviews get skipped under pressure. | **Hooks + RIPER-CAT** — Claude Code hooks that block `git commit` until a review marker exists, two edit-time gates that demand the facts before the first edit of a file and guard the protected declarative files (allowlists, suppression lists, CI configs) behind an explicit override, plus a PLAN → EXECUTE → REVIEW mode system with auto-transitions. |
...
| [`05-DEVELOPER-SETUP.md`](./05-DEVELOPER-SETUP.md) + [`user-settings.template.json`](./user-settings.template.json) + [`merge-user-settings.py`](./merge-user-settings.py) | Per-developer | The half of Claude Code config a repo cannot carry — auto mode, the classifier's `autoMode.environment`, per-model effort. Placeholder template + one-command merge script, run once per developer. |
| [`agent-model-tier.py`](./agent-model-tier.py) + [`agent-model-tiers.template.json`](./agent-model-tiers.template.json) | Fleet tiers | Preferred/fallback model+effort per agent, recorded once and applied mechanically — `check` fails loudly on drift, `apply fallback` / `apply preferred` switch the fleet when a quota wall hits. |
| [`init.sh`](./init.sh) | — | One-command installer (`curl ... | bash`) |
| [`skills/`](./skills/) | Reusable skills | Drop-in ready `.claude/skills/` files. See below. |
| [`CHANGELOG.md`](./CHANGELOG.md) | — | What's new in this snapshot (auto mode replaces "bypass + deny list", the per-developer setup files, Fable 5.1 as the default agent model, the fleet tier manifest, per-session review markers + gitleaks gate + review-gate self-test, `/review-*` skill family, etc.) |
...
- [`skills/close/`](./skills/close/) — `/close` winds a window down without losing anything: stashes its own edits by explicit path, returns its in-progress board tasks, ends the session with a handoff, files what needs a person (see `02-VIBE-BOARD.md` § Human tasks on the same board)
...
**Fleet self-improvement** (behavioral counterpart to the structural audits):
- [`skills/self-improve/`](./skills/self-improve/) — `/self-improve` mines review findings + worker-batch outcomes for RECURRING patterns and encodes preventive guardrails (LESSONS.md rules applied; agent/CLAUDE.md hardening proposed). Governing rule: a one-off is never a rule.
...
The framework is **project-type-agnostic** — works for web apps, CLIs, infrastructure, data pipelines. The examples in the bootstrap are drawn from a production SaaS codebase but the patterns transfer cleanly.
