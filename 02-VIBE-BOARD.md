...
The 16 tools most agents use (the server registers 21 in total; the shared-memory tools and `board_bulk_delete_activity` are among the rest — an MCP `tools/list` call enumerates them all). Implementations live in the cloned [`HuntsDesk/ve-vibe-board`](https://github.com/HuntsDesk/ve-vibe-board) source under `src/tools/` (projects.ts, tasks.ts, sessions.ts, activity.ts). The 5 **bolded** tools below are additions from the 2026-04 release; 2.1.0 (2026-09) adds the `deviation_flagged` activity action; 2.2.0 (2026-09-16) adds the two `gate_events` tools below, the `scripts/ingest-gate-events.mjs` push script and 7 composite indexes for that collection. Run `git pull && npm run build` in your cloned ve-vibe-board dir, then reconnect the MCP (a running server keeps the old schema), to pick them up.
