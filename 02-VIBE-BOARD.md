...
- **21 MCP tools** for task/project/session management, all of them tabulated below (see [tool reference](#mcp-tool-reference)) — 16 in day-to-day use plus five for shared memory and activity-log cleanup. An MCP `tools/list` call against your own build is the authority; don't trust a count in a doc over the running server
...
**Since 2.2.0 the clone ships every index as `firestore.indexes.json`** — 12 in all: the two above, three more the other collections use, and 7 for `gate_events` covering the single-filter queries plus `dev+hook` and `hook+event`. Deploying that file creates them all at once and is the recommended path:

```bash
# from the ve-vibe-board clone
firebase deploy --only firestore:indexes --project YOUR_PROJECT_ID
```

`board_query_gate_events` can combine more filters than those 7 cover (for example `dev+event`, or a path filter with any other). A missing index does not throw: the tool returns `error: missing_composite_index` with the console link that creates it, so add such indexes as you hit them.
...
```json
{
  "permissions": {
    "allow": [
      ...
      "mcp__vibe-board__board_query_gate_events",
      "mcp__vibe-board__board_ingest_gate_events"
    ]
  },
...
All 21 tools the server registers: the 16 most agents use, then the shared-memory and activity-cleanup five. An MCP `tools/list` call against your own build is the authority if this page and the server ever disagree. Implementations live in the cloned [`HuntsDesk/ve-vibe-board`](https://github.com/HuntsDesk/ve-vibe-board) source under `src/tools/` (projects.ts, tasks.ts, sessions.ts, activity.ts). The 5 **bolded** tools below are additions from the 2026-04 release; 2.1.0 (2026-09) adds the `deviation_flagged` activity action; 2.2.0 (2026-09-16) adds the two `gate_events` tools below, the `scripts/ingest-gate-events.mjs` push script and 7 composite indexes for that collection. Run `git pull && npm run build` in your cloned ve-vibe-board dir, then reconnect the MCP (a running server keeps the old schema), to pick them up.
...
| `board_create_task` | Create a task (supports `parent_task_id` for subtasks). **Set the native `task_type`** — `story` / `design` / `task` / `bug` / `chore` / `investigation` — plus `story_id` / `design_id` for 3-tier linkage. Never encode the type as a `[STORY]`-style title prefix: that is a pre-field legacy convention, invisible to the `task_type` filter on `board_get_tasks`, and it drifts from the field the moment a title is edited. |
| `board_update_task` | Update status/priority/assignment/RIPER mode/project_id. **Since 2.0.0**: closing (status=done) a task with open children REJECTS unless `on_open_children: "close_all" \| "detach" \| "leave_attached"` is passed; `leave_attached` requires non-empty `reason`. Closing a child returns `parent_now_empty: { id, title }` advisory when parent has no open siblings left. |
| **`board_bulk_update_tasks`** | Apply same update (project_id/status/priority/agent) to 1-100 tasks atomically. All-or-nothing: preflight fails if any task missing. **Since 2.0.0**: bulk-closing tasks with open children REJECTS unless `on_open_children: "leave_attached"` + `reason` is passed (bulk cascade `close_all`/`detach` not supported — use single-task `board_update_task` for those). |
...
| Tool | Purpose |
|---|---|
| `board_log_activity` | Write an activity log entry (action types: created, updated, claimed, blocked, completed, commented, mode_changed, session_started, session_ended, deviation_flagged — the last added in 2.1.0; any other value is rejected by the schema) |
...
| Tool | Purpose |
|------|---------|
| **`board_query_gate_events`** | Read gate-telemetry events across ALL developers from the shared `gate_events` collection. Filter by dev (git email), hook, event, path, since. This is what `/self-improve` reads first, falling back to the local `gate-events.jsonl`. |
| **`board_ingest_gate_events`** | Upload local telemetry lines into `gate_events`. Idempotent: the document id is derived from the event, so the automatic hook path and a manual push never duplicate. The manual path for a window whose hooks did not run. |

### Shared memory

A team-wide memory store in the `shared_memories` collection, separate from tasks. A write is visible to every other agent and developer on their next search (Firestore replication, ~100 ms) — no git pull, no restart. Use it for cross-developer facts that carry a rule and a reason: ops gotchas, schema discoveries, recurring bugs and their fixes. Personal style preferences belong in your own local memory directory, and in-flight task context belongs on a task.

| Tool | Purpose |
|------|---------|
| `board_save_shared_memory` | Save one memory: `title`, `body`, optional `topic` (`ops` / `coding` / `project` / `reference` / `feedback` / `session`, or free-form) and `author`. Make the title specific enough that a later keyword search finds it. |
| `board_search_shared_memories` | Search by `topic` (exact), `query_text` (free text) and `recency_days`, newest first, up to 50 per call. Call it at session start with `recency_days=7`, and mid-session when the question touches a domain someone else may have hit. |
| `board_delete_shared_memory` | Hard-delete one memory by id; `dry_run=true` previews. Prefer rewriting a stale memory's body — deleting discards the history of why it was ever true. |

### Activity-log cleanup

| Tool | Purpose |
|------|---------|
| `board_delete_activity` | Hard-delete one `activity_log` entry by the `id` that `board_get_activity` returned. Irreversible. |
| `board_bulk_delete_activity` | Delete entries matching filters, which AND together. Refuses to run with no filter, so it cannot wipe the collection. Bounded by `max_delete` (default 500, max 5000); over the cap it deletes nothing and returns `truncated: true` so you can confirm the scope first. |
...
Claude Code 2.1.233 removed the todo/task tools by default on Opus 4.8, Sonnet 5, Fable, Mythos and newer, so on a current model there is usually nothing to block (`CLAUDE_CODE_ENABLE_TODO_TOOLS=1` restores them). `block-todowrite.sh` still ships and still denies the tool wherever it exists — on an older model, or in a session that re-enabled it. Either way the board replaces it entirely.
...
| Review/audit produces findings | Parent task per severity tier; a subtask per **high/critical** finding; medium/low findings go into ONE ledger comment on the parent |
...
- **Review findings → board tasks (severity-gated)**: When a review produces findings (FAILs, WARNs, issues, blockers), every finding lands on the board — never only in conversation text. Create one parent task per severity tier (e.g., "Tier 1: BLOCKING items"). **High and critical** findings (BLOCKING/FAIL → `critical`, HIGH/WARN → `high`) each become a subtask under the tier parent via `parent_task_id`, with enough context to fix the issue without re-reading the review. **Medium and low** findings are NOT individual tasks: write them as one ledger comment on the parent (`board_log_activity`, one line per finding with the same file:line context); they become tasks only when picked up or promoted on evidence of live harm. Why: under one-task-per-finding, a month can file three tasks for every one it closes and the open count stops meaning anything.
...
2. Processor categorizes by severity, creates board tasks (parent per tier, subtask per high/critical finding, one ledger comment for medium/low)
...
| `board_create_task` | Create a task with title, description, priority, dependencies, and a native `task_type` (+ `story_id` / `design_id`) |
| `board_update_task` | Update status, priority, assignment, description, dependencies (2.0.0: parent-close requires `on_open_children` directive when children open) |
...
## Human tasks on the same board

The Vibe Board tracks what agents do. Sooner or later an agent needs a *person* to do something — approve a push to main, answer a product question, check an inbox, paste a draft it wrote — and an ask that lives only in a handoff note dies with the session. The `/close` skill (`skills/close/`) files such asks as it winds a window down, and needs somewhere to put them.

You do not need a second system. A human task board fits inside the same Firestore instance as a convention over fields the MCP already has:

- **One project for human work.** `board_create_project(name="Human Inbox")`, or one per person. Agent projects stay agent-only, and a person reads one place.
- **`assigned_agent` is the person.** It is a free-form string and `board_get_tasks` filters on it, so `assigned_agent: "alice"` works today. `board_get_tasks(project_id=<inbox>, assigned_agent="alice", status="todo")` is Alice's list.
- **Link both ways in `metadata`.** The human task carries `vibe_task_ids: [...]`, the agent tasks it is about; each of those carries `human_task_ids: [...]`. `board_update_task` replaces a metadata array rather than appending to it, so read the existing array and write the union. When an agent closes a board task that has `human_task_ids`, it comments on the human task in the same step, so the inbox never goes stale.
- **A due date is `metadata.due`** (an ISO date). There is no dedicated field; a convention is enough to filter on.
- **Look before you file.** `board_get_tasks(project_id=<inbox>, status="todo")`, then compare titles and `vibe_task_ids`. An open task about the same board item is where the new note goes, not a new row.

Humans read it in the Firebase console (above) or by asking any agent "what is in my inbox". If you already run a human tracker — an issue tracker, an admin page in your own app, a spreadsheet — point `/close` Step 5 at that instead. The skill's rules do not change: file only what a person must do, apply your own recommendation first, re-verify the ask before filing, link both directions. Only the write path differs. The reference project keeps a dedicated human-tasks table behind its admin UI with two shell scripts as the agent's write path, and that block is what the kit strips from the canonical skill.

---

## Cost

Firebase Firestore free tier: 50,000 reads and 20,000 writes **per day**, against 1 GB of **total** stored data (storage is a standing allowance, not a daily one — board documents are small text, so the daily operation limits are what you would hit first, if anything). This easily handles hundreds of agent sessions before you'd ever see a bill. Check [Firebase's current pricing page](https://firebase.google.com/pricing) rather than this line — free-tier numbers change.
