...
| `modelSettings` / `alwaysThinkingEnabled` | Effort per model, keyed by full model ID. Opus 5.5 defaults to `medium` and ignores a top-level `effortLevel` in the user file, so without a `claude-opus-5-5` entry the main session runs at `medium`; the template sets it to `high`, the kit's floor for agent work. The `claude-opus-5` entry covers the previous Opus. Change either with `/effort` or the `/model` picker, which write the same key ([model configuration](https://code.claude.com/docs/en/model-config)). |
...
- **A push to the production branch prompts in a terminal session** (the `ask` rule `Bash(git push*main*)` from Phase 6, adjusted to your branch name). Inside an IDE extension or another SDK host the prompt may not be surfaced at all — we have measured one where the push simply ran while `deny` rules still fired. So the prompt is a backstop, not the approval: `riper-cat.md` requires the user's explicit go-ahead in the conversation before a production push, and no prompt means nothing either way. Probe your own environment with `git push no-such-remote main` (it cannot push anything).
