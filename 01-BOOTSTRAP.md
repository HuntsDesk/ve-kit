...
This intercepts `git commit` and blocks it unless a review marker (`/tmp/ve-review-complete`) exists and is fresh. It is the enforcement mechanism for the "no commit without review" rule.
...
**The marker**: after a review completes, `touch /tmp/ve-review-complete` to authorize the commit. The gate consumes it on use, so each marker authorizes one commit. Note `/tmp` is shared across all concurrent Claude Code windows on the machine — that sharing is exactly why the TTL is short, and it means a marker created in one window can authorize a commit in another. If you run many windows, consider namespacing the marker path per project.
...
#   a marker file is created at /tmp/ve-review-complete
...
if printf '%s\n' "$COMMAND" | grep -qE '(^|[;&|])\s*git(\s+-[Cc]\s*\S+)*\s+commit'; then
  ...
  if [ -f "$MARKER" ]; then
    # Check age — allow if < 3600 seconds (1 hour). Markers live in shared /tmp
    # across all concurrent windows; keep the authorization window short.
    # KNOWN LIMITATION: the marker is window-agnostic — with concurrent windows,
    # window B's commit can consume window A's review marker (B wrongly authorized,
    # A re-reviews). Bounded by the 1h TTL; if it bites in practice, namespace the
    # marker by session (hook input JSON carries session_id).
  ...
  # No valid marker — block the commit and instruct auto-review
  echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"REVIEW GATE — code changes require review before commit. AUTO-RUN the review now:\n\n1. Invoke the code-reviewer agent via Task tool (focus: the staged diff).\n2. If the staged diff includes TypeScript/Python/SQL/tests, also invoke test-runner.\n3. If any critical or high findings surface, STOP and surface them to the user for a decision. Do NOT commit.\n4. If findings are low/medium or clean, file them to the board (for low/medium), create the marker with `touch /tmp/ve-review-complete`, and retry the commit immediately.\n\nDo NOT ask the user for permission to run the review — they have standing approval for this flow. Only pause if findings require their input."}}'
  exit 0
fi
...
11. Test that hook scripts run without errors:
    ...
    # (b) Code staged, no review marker -> gate DENIES. Expect JSON containing "deny".
    #     This is the case that proves the hook actually works — (a) alone cannot fail.
    # NOTE: this clears the shared marker. Fine during first-run bootstrap; if you
    # already run several Claude Code windows, skip this line — it would revoke a
    # review another window is holding.
    rm -f /tmp/ve-review-complete && touch _probe.py && git add _probe.py
    echo '{"tool_name":"Bash","tool_input":{"command":"git commit -m test"}}' | .claude/hooks/review-gate.sh
    git rm -q --cached _probe.py && rm -f _probe.py
