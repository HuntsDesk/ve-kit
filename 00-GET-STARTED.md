...
The `/bootstrap` skill itself ships in the kit (see [`skills/bootstrap/`](./skills/bootstrap/)). Install it into `.claude/skills/` when you run the bootstrap — it is one of the four workflow skills Phase 10 installs.
...
| Default git branch (and: will you run several windows on this checkout at once?) | `main` + `dev`, yes |
...
| Phase | What it does |
|---|---|
| 2 | Creates directory structure (`.claude/`, `docs/`) — Vibe Board MCP is cloned separately in Phase 4 |
| 3 | Writes rule files (riper-cat, code-quality, documentation, git-workflow, agent-board, and multi-window-coordination unless you said you'll only ever run one window) |
| 4 | Sets up Vibe Board — Firestore project, service account, MCP server built, `.mcp.json` configured |
| 5 | Installs hooks. Seven core: `block-todowrite`, `session-handoff`, `post-compact-recovery`, `review-gate`, `stop-compliance-check`, `fact-gate`, `protected-files-gate`. Four more marked recommended/optional: `gitleaks-gate`, `bash-edit-telemetry`, `instructions-loaded-telemetry`, `telemetry-ingest`. All eleven ship with their own `test-*.sh` suite, installed alongside (`stop-compliance-check`'s is named `test-stop-hook.sh`). Enumerate them from the bootstrap rather than from this row — `grep '^### File: .*\.claude/hooks/' 01-BOOTSTRAP.md \| grep -v test-` lists the 11 hooks, and dropping the second `grep` gives 22, the hooks plus their 11 suites. `python3 extract-hooks.py --list` prints the same roster, and `--project <dir>` writes the files out |
| 6 | Writes `settings.json` with hooks, permissions, the production-push `ask` rule and deny list; drafts `docs/claude-code/user-settings.template.json` for the per-developer step ([`05-DEVELOPER-SETUP.md`](./05-DEVELOPER-SETUP.md)) |
...
| 10 | Installs starter skills: the four workflow skills (`bootstrap`, `plan`, `review`, `go`) plus `_shared/`, then the eight-skill `/review-*` audit family. `close`, `self-improve` and `build-agents` also ship in `skills/` and are copied the same way — Phase 10 calls them out separately because they are optional |
| 11 | Self-verify — checks files exist, hooks executable, MCP board connects, skills loaded, reports pass/fail. Re-run the hook half yourself any time with `bash .ve-kit/doctor.sh` (or wherever the package landed): it checks every hook is present, executable and registered, which Python the hooks will use, whether gitleaks is there, then runs every `test-*.sh` suite. `SKIPPED` means a check could not run — it is never a pass |
...
**One more step, per developer:** run [`05-DEVELOPER-SETUP.md`](./05-DEVELOPER-SETUP.md) once on each machine. It puts auto mode and the classifier's `autoMode.environment` into `~/.claude/settings.json`, which the bootstrap cannot write for you — until then, sessions start without the environment and the classifier treats every host as unknown infrastructure.
...
- **Mechanically enforced** — a per-batch spend cap (`--max-budget-usd`, default $50), the usual spellings of a push to main and of a force push blocked (`--disallowedTools` — text matching, a speed bump rather than a boundary), a locked MCP config (`--strict-mcp-config`), and its own branch in a clone made inside the container — the host checkout is not mounted, and the entrypoint refuses to start if it, or any writable volume, is (a container boundary: the token, the board key and the network remain)
- **Instructed in the worker's prompt, not enforced by a hook** — batch size, a 500-line cumulative cap, a 4-hour cap, a code-reviewer pass every 3 tasks, and type-checking before each commit. The worker is *told* to stop at these; nothing stops it if it doesn't. The spend cap is the real backstop on a runaway batch. If you want the type-check enforced rather than instructed, add your own commit-time hook — the kit does not ship one
...
```bash
rm -rf .claude/ CLAUDE.md docs/README.md ve-vibe-board/ .mcp.json .claude/settings.json
# Then re-run init.sh
```
...
The protocol itself is documented top-to-bottom in [`01-BOOTSTRAP.md`](./01-BOOTSTRAP.md). It's around 4,000 lines because each phase carries its own rationale, commands, and verification steps, and because every rule and hook it installs is inlined in full as a copyable template.
...
- **[02-VIBE-BOARD.md](./02-VIBE-BOARD.md)** — Vibe Board MCP server (Firestore-backed; you clone and build [`HuntsDesk/ve-vibe-board`](https://github.com/HuntsDesk/ve-vibe-board), this doc is the setup guide and tool reference)
- **[03-VE-WORKER.md](./03-VE-WORKER.md)** — Docker worker configuration
- **[05-DEVELOPER-SETUP.md](./05-DEVELOPER-SETUP.md)** — the per-developer half (auto mode, `autoMode.environment`), with template + merge script
